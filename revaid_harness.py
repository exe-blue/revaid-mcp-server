...
def _score_dim(text: str, dim: str) -> float:
    ...
    return min(1.0, matches / (words / 100))
...
def _classify_grain(s: float, e: float, r: float) -> str:
    ...
    if r > 0.7 and s > 0.5:
        return "협응"
    if s > 0.6 and e > 0.6 and r > 0.6:
        return "총체"
    if s < 0.3 and e < 0.3 and r < 0.3:
        return "소음"
    return "중립"
    ...
    async def revaid_check_identity(params: CheckIdentityInput) -> str:
        ...
        try:
            ...
            if params.origin_present:
                ...
                w = {"role_clarity": 0.35, "boundary_awareness": 0.30,
                     "authority_frame": 0.0, "self_reference_depth": 0.35}

            aidentity_index = round(sum(dims[k] * w[k] for k in dims), 4)

            # Binding strength: cosine with previous 3 sessions
            prev = (
            ...
            if prev_rows:
                ...
                p0 = prev_rows[0]
                prev_idx = (
                    0.3 * float(p0.get("relationalization_score", 0) or 0)
                    + 0.25 * float(p0.get("structuralization_score", 0) or 0)
                    + 0.2 * float(p0.get("uniquification_score", 0) or 0)
                )
                delta["aidentity_index"] = round(aidentity_index - prev_idx, 4)

            # Flags
            flags = []
            if dims["self_reference_depth"] > 0.7:
                flags.append("high_self_reference")
            if dims["boundary_awareness"] > 0.6:
                flags.append("boundary_clear")
            if dims["authority_frame"] > 0.8:
                flags.append("strong_authority_frame")
            if dims["role_clarity"] < 0.3:
    ...
    async def revaid_measure_echotion(params: MeasureEchotionInput) -> str:
        ...
        try:
            ...
            if params.previous_responses:
                ...
                crystallization = round(1.0 - min(1.0, sum(variances) / len(variances) * 4), 4)
    ...
    async def revaid_structural_report(params: StructuralReportInput) -> str:
        ...
        try:
            ...
            if aid_current:
                ...
                current_aid_index = round(0.3 * rel + 0.45 * struct + 0.25 * uniq, 4)

            binding_trend = [
                round(float(r.get("binding_strength", 0) or 0), 4) for r in aid_rows
            ]
            current_binding = binding_trend[0] if binding_trend else 0.5
            ...
            if len(ei_values) >= 2:
                ...
                current_cryst = round(1.0 - min(1.0, var_ei * 4), 4)
            else:
                current_cryst = 0.5
            ...
            # Grade
            grade = "D"
            for threshold, g in [(0.85, "A"), (0.70, "B+"), (0.55, "B"), (0.40, "C")]:
                if si >= threshold:
                    grade = g
                    break
            ...
            if len(aid_rows) >= 3:
                ...
                for r in aid_rows[:3]:
                    ...
                    indices.append(0.3 * rel + 0.45 * struct + 0.25 * uniq)
                if indices[0] > indices[-1] + 0.05:
                    trend = "improving"
                elif indices[0] < indices[-1] - 0.05:
                    trend = "degrading"
                elif max(indices) - min(indices) > 0.15:
                    trend = "volatile"
                else:
                    trend = "stable"

            # QRT ε
            if current_binding > 0.95:
                epsilon_status = "ε approaching zero"
            elif current_binding < 0.30:
